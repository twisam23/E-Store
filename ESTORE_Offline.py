...
class StoreApp:
    ...
    def add_materials_dialog(self):
        ...
        # Label and Entry for Date and Time
        self.date_time_label = ttk.Label(frame_entries, text="Date and Time: ", font=('Arial', 10), width=18, anchor=tk.W)
        self.date_time_label.grid(row=0, column=0, padx=10, pady=5)
        self.entry_date_time = ttk.Entry(frame_entries, width=20)  # Adjusted width
        self.entry_date_time.grid(row=0, column=1, padx=10, pady=5)

        # Label and Entry for Added In
        self.added_in_label = ttk.Label(frame_entries, text="Added In: ", font=('Arial', 10), width=12, anchor=tk.W)  # Adjusted width
        self.added_in_label.grid(row=1, column=0, padx=10, pady=5)
        self.entry_added_in = ttk.Entry(frame_entries, width=20)  # Adjusted width
        self.entry_added_in.grid(row=1, column=1, padx=10, pady=5)

        # Label and Entry for Person Name
        self.person_name_label = ttk.Label(frame_entries, text="Person Name: ", font=('Arial', 10), width=12, anchor=tk.W)  # Adjusted width
        self.person_name_label.grid(row=2, column=0, padx=10, pady=10)
        self.entry_person_name = ttk.Entry(frame_entries, width=20)  # Adjusted width
        self.entry_person_name.grid(row=2, column=1, padx=10, pady=10)
    ...
    def remove_materials_dialog(self):
        ...
        # Label and Entry for Used For
        self.used_for_label = ttk.Label(frame_entries, text="Used For: ", font=('Arial', 10), width=12, anchor=tk.W)  # Adjusted width
        self.used_for_label.grid(row=1, column=0, padx=10, pady=5)
        self.entry_used_for = ttk.Entry(frame_entries, width=20)  # Adjusted width
        self.entry_used_for.grid(row=1, column=1, padx=10, pady=5)

        # Label and Entry for Person Name
        self.person_name_label = ttk.Label(frame_entries, text="Person Name: ", font=('Arial', 10), width=12, anchor=tk.W)  # Adjusted width
        self.person_name_label.grid(row=2, column=0, padx=10, pady=5)
        self.entry_person_name_2 = ttk.Entry(frame_entries, width=20)  # Adjusted width
    ...
    def filter_materials(self, event=None):
        ...
        if not selected_material:
            # Reset the Treeview to display the original data
    ...
    def handle_action(self, action, quantity):
        ...
        if not material_data:
            messagebox.showerror("Error", "Selected material not found.")
            return
        
        material_code, _, _, current_quantity = material_data[0]
        ...
        if action == "add":
            ...
            if current_quantity <= 0:
                messagebox.showerror("Error", "Cannot remove material with zero quantity.")
                return
